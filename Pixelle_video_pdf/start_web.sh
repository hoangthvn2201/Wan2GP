#!/bin/bash
# Start Pixelle-Video PDF → Video Web UI
#
...
echo "🚀 Starting Pixelle-Video PDF → Video Web UI..."
echo ""
...
# Use a different port than the original app (8501) and the scene-by-scene
# app (8502) so all three can run together.
streamlit run "$SCRIPT_DIR/web/app.py" --server.port "${PORT:-8503}"
