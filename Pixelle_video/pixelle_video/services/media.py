...
from Pixelle_video.pixelle_video.models.media import MediaResult
...
class MediaService(ComfyBaseService):
    ...
    def _scan_workflows(self):
        ...
        """
        from Pixelle_video.pixelle_video.utils.os_util import list_resource_dirs, list_resource_files, get_resource_path
    ...
    async def __call__(
        ...
        **params
        ...
        # Wan2GP workflows run in-process (no ComfyUI / RunningHub involved)
        if workflow_info["source"] == "wan2gp":
            return await self._execute_wan2gp(
                workflow_info,
                prompt=prompt,
    ...
    async def _execute_wan2gp(
        ...
        """
        Execute a wan2gp workflow descriptor by loading the model in-process
        through the WanGP Python API (no ComfyUI server needed).
        """
        ...
        # The descriptor states what it generates; the caller's media_type
        # (derived from the workflow filename) should match, but the
        # descriptor wins since it knows the actual model.
        descriptor_media_type = descriptor.get("media_type", media_type)

        client = get_wan2gp_client()
        output_path = await client.generate(
            descriptor,
            prompt,
        ...
        if descriptor_media_type == "video":
            return MediaResult(media_type="video", url=output_path, duration=None)
