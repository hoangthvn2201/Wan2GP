...
class LLMConfig(BaseModel):
    ...
    enable_thinking: Optional[bool] = Field(
        default=None,
        description=(
            "Control reasoning/'thinking' mode for reasoning models (Qwen3, "
            "DeepSeek-R1, etc.). Set to False to disable chain-of-thought so the "
            "model emits answer content directly (recommended for JSON/structured "
            "tasks). None = leave provider default untouched (e.g. for OpenAI)."
        ),
    )
...
class Wan2GPConfig(BaseModel):
    """Wan2GP in-process generation configuration (workflows/wan2gp/*.json)"""
    root: Optional[str] = Field(
        default=None,
        description="Path to the Wan2GP installation (default: auto-detect parent repository)"
    )
    cli_args: List[str] = Field(
        default_factory=list,
        description="WanGP startup flags, e.g. ['--attention', 'sdpa', '--profile', '4']"
    )
    output_dir: Optional[str] = Field(
        default=None,
        description="Override for WanGP output directory (default: WanGP's own outputs folder)"
    )
...
class PixelleVideoConfig(BaseModel):
    ...
    wan2gp: Wan2GPConfig = Field(default_factory=Wan2GPConfig)
