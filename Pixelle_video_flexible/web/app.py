...
"""
Pixelle-Video Flexible (generate-or-search) Web UI - Main Entry Point

A wizard that turns a topic into a narrated short video where an LLM
orchestrator decides PER SCENE whether the visual is AI-generated (WanGP /
ComfyUI) or real stock media searched on Pexels/Pixabay:
topic → script → media plan (review/edit) → source media (candidate gallery)
→ scenes → final.
...
flexible pipeline (`flexvid` package) live in this folder.

sys.path layout (order matters):
    1. this folder                       -> `web` (this UI), `flexvid` (flex engine)
...
# 1. This project folder (for `web` and `flexvid` packages).
...
st.set_page_config(
    page_title="Pixelle-Video · Flexible Video",
    page_icon="🎬",
    layout="wide",
